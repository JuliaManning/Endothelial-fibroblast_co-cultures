...
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/EC PCA coloured by batch sex removed combat.tiff",
...
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/EC PCA coloured by diagnosis sex removed combat.tiff",
     width = 600, height = 800)
...
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/EC pairsplot coloured by EC donor sex removed combat.tiff",
...
pheatmap(subset, scale = "row")

col_fun <- colorRamp2(breaks = seq(-4, 4, length.out=101),
                      colorRampPalette(rev(brewer.pal(n=11, "RdYlBu")))(101))

tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/EC 100 variable genes heatmap no sex genes combat batch corrected with filter z-score.tiff",
...
# genes from the Biplot 
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/Genes from biplot combat.tiff", 
...
e <- plotCounts(dds, gene='ENSG00000133048', intgroup="Diagnosis", returnData = TRUE) # CHI3L1
f <- plotCounts(dds, gene='ENSG00000187608', intgroup="Diagnosis", returnData = TRUE) # ISG15
g <- plotCounts(dds, gene='ENSG00000198888', intgroup="Diagnosis", returnData = TRUE) # MT-ND1
h <- plotCounts(dds, gene='ENSG00000198899', intgroup="Diagnosis", returnData = TRUE) # MT-ATP6
i <- plotCounts(dds, gene='ENSG00000228253', intgroup="Diagnosis", returnData = TRUE) # MT-ATP8
j <- plotCounts(dds, gene='ENSG00000198695', intgroup="Diagnosis", returnData = TRUE) # MT-ND6

A <- ggplot(a, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("TGFBI")+
  ylab("Normalised count")
B <- ggplot(b, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("IGFBP5")+
  ylab("Normalised count")
C <- ggplot(c, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("TNFAIP6")+
  ylab("Normalised count")
D <- ggplot(d, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("COL6A3")+
  ylab("Normalised count")
E <- ggplot(e, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("CHI3L1")+
  ylab("Normalised count")
FF <- ggplot(f, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("ISG15")+
  ylab("Normalised count")
G <- ggplot(g, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("MT-ND1")+
  ylab("Normalised count")
H <- ggplot(h, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("MT-ATP6")+
  ylab("Normalised count")
I <- ggplot(i, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("MT-ATP8")+
  ylab("Normalised count")
J <- ggplot(j, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("MT-ND6")+
  ylab("Normalised count")
cowplot::plot_grid(A, B, C, D, E, FF, G, H, I, J,
                   ncol = 3, nrow = 4)
dev.off()
...
save(dds, p, design, adjusted, diagnosis_col, EC_sex_col, eccountdata, ecsampleinfo, fibroblast_sex_col, my_metadata, pcDat, subset, 
