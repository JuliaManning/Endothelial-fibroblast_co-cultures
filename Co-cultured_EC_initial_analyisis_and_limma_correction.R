...
# genes from the PCA
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/Genes from heatmap.tiff", width = 600, height = 600)
...
J <- ggplot(j, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  ...
  ggtitle("SNORD63")+
  ylab("Normalised count")
K <- ggplot(k, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("AC034195.1")+
  ylab("Normalised count")
L <- ggplot(l, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
  ggtitle("GREM")+
...
load("~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/Outputs/EC/EC data no filter sex removed before limma correction.RData")
...
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/EC 100 variable genes heatmap no sex genes limma batch corrected with filter z-score.tiff",
...
tiff(filename = "~/OneDrive/Documents/PhD/Projects/Endothelial RNA seq/EC only output/Final/Genes from biplot limma.tiff", 
...
f <- plotCounts(dds, gene='ENSG00000210144', intgroup="Diagnosis", returnData = TRUE) # MT-TY
g <- plotCounts(dds, gene='ENSG00000210191', intgroup="Diagnosis", returnData = TRUE) # MT-TL2
...
j <- plotCounts(dds, gene='ENSG00000254332', intgroup="Diagnosis", returnData = TRUE) # GS1-44D20.1
A <- ggplot(a, aes(x=Diagnosis, y=count, colour = ecsampleinfo$Diagnosis)) + 
  theme(legend.position = "none") +
  geom_point(position=position_jitter(w=0.1,h=0)) + 
  scale_y_log10()+
